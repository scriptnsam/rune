...
import (
	...
	"github.com/scriptnsam/rune/ingest/middleware"
...
func main() {
	...
	// Start Cache Refresher (Runs every 5 minutes)
	cache.StartCacheRefresher(5 * time.Minute)

	app := fiber.New(fiber.Config{AppName: "Rune Ingestion Engine v0.1"})
	app.Use(cors.New(cors.Config{
		AllowOrigins: []string{"http://localhost:3000"},
		AllowMethods: []string{"Origin", "Content-Type", "Accept", "x-api-key"},
	}))
	...
	app.Post("/api/v1/ingest", middleware.Protect, handlers.InjestLog)
